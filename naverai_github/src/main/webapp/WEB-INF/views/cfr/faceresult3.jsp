...
$(document).ready(function() {
	...
	for(var i = 0; i < json.info.faceCount; i++){		
		...
		$("h1").append("왼쪽입끝 = " + json.faces[i].landmark.leftMouth.x + " : " + json.faces[i].landmark.leftMouth.y + "<br>");
		$("h1").append("오른쪽입끝 = " + json.faces[i].landmark.rightMouth.x + " : " + json.faces[i].landmark.rightMouth.y + "<br>");
		}
	...
	var myimage = new Image(); // 이미지객체 비어있다ㅣ.
	myimage.src = "/naverai/${param.image}"; //이미지객체 비어있지않다.
	myimage.onload = function(){
		...
		var colors = ["red", "blue", "yello", "green"];
		...
		for(var i = 0; i < json.info.faceCount; i++){		
			...
			//landmark 있으면 왼눈, 오른눈, 코, 양끝입술 빨간색으로 표시.
			if(json.faces[i].landmark != null){
